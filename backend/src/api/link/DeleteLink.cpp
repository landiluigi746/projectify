...
namespace projectify::API
{
    void DeleteLink(const crow::request& req, crow::response& res)
    {
        ...
        if(!requestData.has("projectID") || requestData["projectID"].t() != crow::json::type::Number ||
            !requestData.has("linkID") || requestData["linkID"].t() != crow::json::type::Number)
        {
            res.code = crow::status::BAD_REQUEST;
            res.end("You need to provide a valid JSON containing name and an integer projectID and an integer linkID");
            return;
        }
        ...
        const auto linkID = requestData["linkID"].i();

        const Database::Result result = Database::DeleteLink(userID, projectID, linkID);

        switch(result)
        {
            case projectify::Database::Result::NOT_FOUND:
                res.code = crow::status::NOT_FOUND;
                res.end("Couldn't find requested user/project/link");
                break;
